...
const Seller_Shop_Info = ({ nextPage }) => {
	...
	const validate = () => {
		...
		!validEmail.test(contactEmail)
			? err.push("Please enter valid contact email")
			: "";
		!orderEmail ? err.push(`Please Enter order email`) : "";
		!validEmail.test(orderEmail)
			? err.push("Please enter valid order email")
			: "";
		!returnEmail ? err.push(`Please Enter return email`) : "";
		!validEmail.test(returnEmail)
			? err.push("Please enter valid return email")
			: "";
		!customerServiceEmail
			? err.push(`Please Enter customer service email`)
			: "";
		!validEmail.test(customerServiceEmail)
			? err.push("Please enter valid service email")
			: "";
	...
	const submit = async (e) => {
		...
		const af = proofOfAddressFrontView
			? await uploadFiles(proofOfAddressFrontView)
			: [];
		const ab = proofOfAddressBackView
			? await uploadFiles(proofOfAddressBackView)
			: [];
								...
								<ol role="list" className="assessment-list assessment-list--1">
									<li>
										...
										<div className="account-form-1 mb-0">
											...
											<div className="input-x input-x--flex">
												{!editPhone ? (
													<div className={`${!editPhone ? "" : " hidden"}`}>
														{phone ? phone : "2-(056)634-6566"}
													</div>
												) : (
													<input
										...
										<div className="assessment-list-content">
											...
											<div className="account-form-1 mb-0">
												...
												<input
													type="tel"
													onChange={(e) =>
														setCustomerServiceEmail(e.target.value)
													}
									...
									<li>
										...
										<div className="product-add-block">
											...
											<input
												type="file"
												onChange={(e) => setAddressFront(e.target.files[0])}
												style={{ display: "none" }}
												id="address-frontview"
											/>
											<input
												type="file"
												style={{ display: "none" }}
												onChange={(e) => setAddressBack(e.target.files[0])}
												id="address-backview"
											/>
											...
											>
												<label className="button" htmlFor="id-frontview">
													Upload Front View
												</label>

												<label className="button" htmlFor="id-backview">
													Upload Back View
												</label>
											</div>
											...
											<div style={imgStyle}>
												{addressFront && (
													<img
														...
														width="220"
														height="240"
														src={URL.createObjectURL(addressFront)}
														style={{ borderRadius: "10px" }}
														alt="address - front"
													/>
												)}
												{addressBack && (
													<img
														...
														width="220"
														height="240"
														src={URL.createObjectURL(addressBack)}
														style={{ borderRadius: "10px" }}
														alt="address-back"
													/>
												)}
											...
											>
												<label className="button" htmlFor="address-frontview">
													Upload Front View
												</label>
												<label className="button" htmlFor="address-backview">
													Upload Back View
												</label>
											</div>
										</div>
